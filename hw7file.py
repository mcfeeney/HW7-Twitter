...
## Any names of people you worked with on this assignment: went to office hours 
...
file_name= "cached_file.json"
try:
	file_open= open(file_name, "r")
	file_contents= file_open.read()
	twitter_dictionary= json.loads(file_contents)
	file_open.close()
except:
	twitter_dictionary= {}
...
def get_twitter_data(term):
	if term in twitter_dictionary:
		print ("Using Cache")
	else:
		print ("Fetching")
		tweets= api.search(q= term)
		twitter_dictionary[term] = tweets
		cache_file = open(file_name, "w")
		cache_file.write(json.dumps(twitter_dictionary))
		cache_file.close()
	return twitter_dictionary
...
for x in range(3):
	...
	get_twitter_data(search)

## 4. With what you learn from the data -- e.g. how exactly to find the 
##		text of each tweet in the big nested structure -- write code to print out 
## 		content from 5 tweets, as shown in the linked example.

	some_dict= (twitter_dictionary[search]['statuses'][:5])
